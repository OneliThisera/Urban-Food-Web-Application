...
if (isset($_POST['update'])) {
    ...
    try {
        $sql_update = "UPDATE URBAN_SUPPLIERS 
                       SET SUPPLIER_NAME = :supplier_name, EMAIL = :email, PASSWORD = :password, ADDRESS = :address, PHONE = :phone 
                       WHERE SUPPLIER_ID = :supplier_id";
        executeQuery($conn, $sql_update, [
            ':supplier_name' => $supplier_name,
            ':email' => $email,
            ':password' => $password,
            ':address' => $address,
...
if (isset($_POST['delete_supplier'])) {
    ...
    try {
        $sql_delete = "DELETE FROM URBAN_SUPPLIERS WHERE SUPPLIER_ID = :supplier_id";
        executeQuery($conn, $sql_delete, [':supplier_id' => $supplier_id]);
            ...
            <table class="table table-bordered table-hover table-striped align-middle">
                ...
                <?php foreach ($suppliers as $supplier): ?>
                    <tr class="text-center">
                        ...
                        <td>
                            <!-- Update Form -->
                            <a href="update_supplier.php?supplier_id=<?php echo $supplier['SUPPLIER_ID']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>

                            <!-- Delete Form -->
                            <form method="POST" action="Supplier.php" style="display:inline;">
                                <input type="hidden" name="supplier_id" value="<?php echo $supplier['SUPPLIER_ID']; ?>">
                                <button type="submit" name="delete_supplier" class="btn btn-danger btn-sm"><i class="fas fa-trash"></i> Delete</button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; ?>
